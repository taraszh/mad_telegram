...
func (u *User32) TypeMessage(text string) {
	const VK_ENTER = 0x0D
	const VK_SHIFT = 0x10

	for _, r := range text {
		if r == '\n' {
			// Simulate pressing the Shift-Enter key
			u.SendKeyDown(VK_SHIFT)
			time.Sleep(15 * time.Millisecond)
			u.SendKeyDown(VK_ENTER)
			time.Sleep(15 * time.Millisecond)
			u.SendKeyUp(VK_ENTER)
			time.Sleep(15 * time.Millisecond)
			u.SendKeyUp(VK_SHIFT)

			println("Simulated Enter key press")
			continue
		}
